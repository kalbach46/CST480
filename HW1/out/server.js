...
async function validAuthorID(author_id) {
    let out = await db.all(`SELECT id FROM authors WHERE id=${author_id}`);
    return out.length > 0;
}
async function validBookID(id) {
...
async function deleteBook(id) {
    return await db.run(`DELETE FROM books WHERE id=${id}`);
}
async function deleteAuthor(id) {
    return await db.run(`DELETE FROM authors WHERE id=${id}`);
}
...
async function getAuthorByID(id) {
    return await db.get(`SELECT * FROM authors WHERE id=${id}`);
}
...
app.get("/getBookByGenre", async (req, res) => {
